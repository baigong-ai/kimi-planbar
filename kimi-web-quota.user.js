...
(function () {
  ...
  //
  // v0.38 note: the server no longer requires auth on REST routes at all, and
  // the frontend now DELETES expired credentials from localStorage (previously
  // it kept them). So a perfectly healthy web UI often has no stored
  // credential — gating on the credential (as pre-1.7 versions did) made the
  // badge silently never appear. The script now probes the endpoint itself
  // (see the boot logic at the bottom) and falls back to an unauthenticated
  // request when no credential is stored.
  ...
  async function fetchUsage(tok) {
    ...
    return fetch('/api/v1/oauth/usage', { headers });
  }

  // Try each stored credential until one isn't rejected; fall back to a bare
  // unauthenticated request (accepted since Kimi Code 0.38, and the only
  // option when the frontend has already deleted an expired credential).
  async function fetchUsageAny() {
    ...
    const bare = await fetchUsage(null);
    return bare.ok ? bare : r || bare;
  }

  // Boot gate: no stored credential used to mean "not a Kimi Code web UI —
  // stay out of the way". Since v0.38 that heuristic is wrong (see above), so
  // probe the endpoint instead: only a Kimi Code web server answers with the
  // usage payload shape, so other local sites on matched origins stay
  // untouched.
  (async () => {
    if (credCandidates().length === 0) {
      try {
        ...
        if (!j || !j.data || j.data.kind !== 'ok') return;
      } catch {
        return;
      }
    }
